...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let currentViewMode = "split"; // split, pt, en
    let searchQuery = "";
    
    // Loaded completed exercises from localStorage
    ...
    if (localStorage.getItem("theme") === "light") {
        ...
        themeToggleBtn.innerHTML = '<span class="theme-toggle-icon">🌙</span>';
    } else {
        document.body.classList.add("dark-theme");
        document.body.classList.remove("light-theme");
        themeToggleBtn.innerHTML = '<span class="theme-toggle-icon">☀️</span>';
    }
    ...
    // Populate Sidebar Themes with counts
    const getThemesCounts = () => {
        const counts = {};
        exercisesData.forEach(ex => {
            counts[ex.theme] = (counts[ex.theme] || 0) + 1;
        });
        return counts;
    };
    
    const renderThemesMenu = () => {
        const counts = getThemesCounts();
        const container = document.getElementById("themes-list");
        // Clear except first (All)
        const allItem = container.firstElementChild;
        container.innerHTML = "";
        container.appendChild(allItem);
        
        // Update All counter
        document.getElementById("count-all").textContent = exercisesData.length;
        
        Object.entries(counts).forEach(([themeName, count]) => {
            const li = document.createElement("li");
            li.className = "menu-item";
            li.setAttribute("data-theme", themeName);
            
            li.innerHTML = `
                <span class="menu-item-text">${themeName}</span>
    ...
    viewModeToggle.addEventListener("click", (e) => {
        ...
        // Update layout on all existing cards
        const cards = document.querySelectorAll(".exercise-card");
        cards.forEach(card => {
            const body = card.querySelector(".card-body");
            body.className = `card-body view-${currentViewMode}`;
        });
    });
    ...
    const renderExercises = () => {
        ...
        const filtered = exercisesData.filter(ex => {
            // Theme filter
            if (currentThemeFilter !== "all" && ex.theme !== currentThemeFilter) return false;
            
            // Search query
            if (searchQuery) {
                const matchEn = ex.en.toLowerCase().includes(searchQuery);
                const matchPt = ex.pt.toLowerCase().includes(searchQuery);
                const matchNum = ex.num.includes(searchQuery);
                const matchSec = ex.section.toLowerCase().includes(searchQuery);
                return matchEn || matchPt || matchNum || matchSec;
            }
        ...
        statCompleted.textContent = completedExercises.length;
        ...
        filtered.forEach(ex => {
            ...
            const notes = personalNotes[ex.id] || "";
            ...
            card.innerHTML = `
                <header class="card-header">
                    <div class="card-meta">
                        <span class="difficulty-badge ${ex.difficulty.toLowerCase()}">${ex.difficulty}</span>
                        <span class="section-tag">${ex.section}</span>
                    </div>
                    <div class="card-title-row">
                        <h3 class="exercise-title">Exercício ${ex.num}</h3>
                        <div class="card-actions">
                            <button class="action-btn resolve-btn ${isCompleted ? 'active' : ''}" title="Marcar como resolvido">
                                <span class="action-icon">${isCompleted ? '✅' : '⬜'}</span>
                            </button>
                            <button class="action-btn copy-btn" title="Copiar enunciado em Markdown">
                                <span class="action-icon">📋</span>
                            </button>
                            <button class="action-btn open-ai-btn" title="Ajuda da IA">
                                <span class="action-icon">🤖</span>
                            </button>
                        </div>
                    </div>
                ...
                <div class="card-body view-${currentViewMode}">
                    <div class="lang-block lang-en">
                        <div class="lang-label"><span class="flag">🇺🇸</span> ENUNCIADO ORIGINAL (EN)</div>
                        <div class="lang-text math-content">${ex.en}</div>
                    </div>
                    <div class="lang-block lang-pt">
                        <div class="lang-label"><span class="flag">🇧🇷</span> VERSÃO TRADUZIDA (PT)</div>
                        <div class="lang-text math-content">${ex.pt}</div>
                    </div>
                </div>

                <div class="card-collapsible-section">
                    <button class="collapsible-header-btn">
                        <span>📝 Minhas Anotações / Solução</span>
                        <span class="arrow-icon">▼</span>
                    </button>
                    <div class="collapsible-content">
                        <textarea placeholder="Escreva aqui sua solução ou anotações... (Salva automaticamente)" class="solution-notes">${notes}</textarea>
                        <div class="notes-status-row">
                            <span class="autosave-tag">Salvo localmente</span>
                        </div>
                    </div>
                </div>

                <div class="card-collapsible-section ai-section">
                    <button class="collapsible-header-btn ai-btn">
                        <span>🤖 Ajuda da IA (Tutor)</span>
                        <span class="arrow-icon">▼</span>
                    </button>
                    <div class="collapsible-content ai-content">
                        <div class="ai-chat-history" id="chat-${ex.id}" style="padding: 16px; height: 250px; overflow-y: auto; display: flex; flex-direction: column; gap: 12px; background: var(--bg-card);">
                            <div class="chat-message ai" style="background: var(--accent-glow); padding: 10px 14px; border-radius: var(--border-radius-sm); border: 1px solid var(--border-hover); font-size: 13.5px; align-self: flex-start;">Olá! Sou seu tutor baseado em Antigravity. Como posso te ajudar com este exercício hoje?</div>
                        </div>
                        <div class="ai-chat-input-row" style="padding: 12px 16px; border-top: 1px solid var(--border); display: flex; gap: 8px; background: var(--bg-secondary);">
                            <input type="text" id="input-${ex.id}" placeholder="Pergunte sobre o exercício..." style="flex: 1; padding: 10px; border: 1px solid var(--border); border-radius: var(--border-radius-sm); background: var(--textarea-bg); color: var(--text-primary); outline: none;">
                            <button class="send-ai-btn" data-ex="${ex.id}" style="padding: 10px 16px; background: var(--accent); color: white; border: none; border-radius: var(--border-radius-sm); cursor: pointer; font-weight: 600;">Enviar</button>
                        </div>
                    </div>
                </div>
            `;
            ...
            resolveBtn.addEventListener("click", () => {
                if (completedExercises.includes(ex.id)) {
                    completedExercises = completedExercises.filter(id => id !== ex.id);
                    resolveBtn.classList.remove("active");
                    resolveBtn.querySelector(".action-icon").textContent = "⬜";
                    card.classList.remove("completed");
                } else {
                    completedExercises.push(ex.id);
                    resolveBtn.classList.add("active");
                    resolveBtn.querySelector(".action-icon").textContent = "✅";
            ...
            copyBtn.addEventListener("click", () => {
                const markdownText = `### Exercício ${ex.num} (${ex.section})\n\n**Original (EN):**\n${ex.en.replace(/\\n/g, '\n')}\n\n**Tradução (PT):**\n${ex.pt.replace(/\\n/g, '\n')}`;
                navigator.clipboard.writeText(markdownText).then(() => {
                    copyBtn.title = "Copiado!";
                    copyBtn.querySelector(".action-icon").textContent = "✔️";
                    setTimeout(() => {
                        copyBtn.title = "Copiar enunciado em Markdown";
                        copyBtn.querySelector(".action-icon").textContent = "📋";
                    }, 2000);
                });
            });

            // Collapsible Notes Listener
            const notesBtn = card.querySelector(".collapsible-header-btn:not(.ai-btn)");
            const collContent = card.querySelector(".collapsible-content:not(.ai-content)");
            notesBtn.addEventListener("click", () => {
                notesBtn.classList.toggle("open");
                collContent.classList.toggle("open");
            });
            ...
            const toggleAi = () => {
                aiBtn.classList.toggle("open");
                aiContent.classList.toggle("open");
                if (aiContent.classList.contains("open")) {
                    aiContent.style.maxHeight = "400px";
                } else {
            ...
            const openAiBtnTop = card.querySelector(".open-ai-btn");
            openAiBtnTop.addEventListener("click", () => {
                if (!aiContent.classList.contains("open")) {
                    toggleAi();
                }
                setTimeout(() => card.querySelector(`#input-${ex.id}`).focus(), 300);
            });

            // AI Chat Send Message
            const sendAiBtn = card.querySelector(".send-ai-btn");
            const aiInput = card.querySelector(`#input-${ex.id}`);
            const chatHistory = card.querySelector(`#chat-${ex.id}`);
            ...
            const sendMessageToAi = async () => {
                ...
                userMsg.style.cssText = "background: var(--bg-secondary); padding: 10px 14px; border-radius: var(--border-radius-sm); font-size: 13.5px; align-self: flex-end; max-width: 85%;";
                ...
                aiMsg.style.cssText = "background: var(--accent-glow); padding: 10px 14px; border-radius: var(--border-radius-sm); border: 1px solid var(--border-hover); font-size: 13.5px; align-self: flex-start; max-width: 85%; white-space: pre-wrap;";
                aiMsg.textContent = "...";
                ...
                try {
                    // Send to backend
                    const contextPrefix = `[Contexto - Exercício ${ex.num}: ${ex.pt}]\n\nAluno diz: `;
                    const response = await fetch("/api/chat", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ message: contextPrefix + message, session_id: ex.id })
                    });
                    ...
                    aiMsg.textContent = "Funcionalidade de IA indisponível na versão estática (GitHub Pages).";
                    aiMsg.style.color = "var(--difficulty-hard)";
                }
            };
            ...
            const notesTextarea = card.querySelector(".solution-notes");
            notesTextarea.addEventListener("input", (e) => {
                personalNotes[ex.id] = e.target.value;
                localStorage.setItem("personalNotes", JSON.stringify(personalNotes));
                
                const autosaveTag = card.querySelector(".autosave-tag");
                autosaveTag.textContent = "Digitando...";
                
                // Debounced saved tag
                clearTimeout(notesTextarea.saveTimeout);
                notesTextarea.saveTimeout = setTimeout(() => {
                    autosaveTag.textContent = "Salvo localmente";
                }, 1000);
            });
